...
output_json = "captions_finetuned.json"
output_csv = "captions_finetuned.csv"
fixed_list_file = "/home/amal.saqib/Downloads/CV703_FoodGen_Project/synced_fixed_image_list.txt"
model_path = "./instructblip_finetuned_last2_decoder_final"
checkpoint_json = "captions_finetuned_checkpoint.json"
checkpoint_csv = "captions_finetuned_checkpoint.csv"
...
class FoodDataset(Dataset):
    def __init__(self, root_dir, fixed_list_path, transform=None):
        ...
        #             self.labels.append(label)
        #             self.filenames.append(file)
        # print("✅ Total filenames in list:", len(filenames))
        # print("✅ Total images found:", len(self.image_paths))
        # print("⚠️ Missing files:")
        # missing = set(filenames) - set(self.filenames)
        # for name in list(missing)[:10]:  # only show first 10
        #     print("-", name)


        # missing = set(filenames) - set(self.filenames)
        # if missing:
        #     print(f"⚠️ Warning: {len(missing)} files were not found. Continuing with the remaining {len(self.image_paths)} images.")

        # #assert len(self.image_paths) == len(filenames), "Mismatch in fixed image list and available data."
...
# === Main Pipeline ===
dataset = FoodDataset(data_root, fixed_list_file, transform=transform)
dataloader = DataLoader(dataset, batch_size=8, shuffle=False)

if len(dataset) == 0:
    raise ValueError("Dataset is empty. Check your list and path.")
