...
# processor = InstructBlipProcessor.from_pretrained(model_path, legacy=True)
...
# === CONFIG ===
output_json = "/home/amal.saqib/Downloads/CV703_FoodGen_Project/finetuning_experiments/experiment_2_2decoder/captions_finetuned_final_food500_test.json"
output_csv = "/home/amal.saqib/Downloads/CV703_FoodGen_Project/finetuning_experiments/experiment_2_2decoder/captions_finetuned_final_food500_test.csv"
model_path = "/home/amal.saqib/Downloads/CV703_FoodGen_Project/finetuning_experiments/experiment_2_2decoder/instructblip_finetuned_last2_decoder_final"
checkpoint_json = "/home/amal.saqib/Downloads/CV703_FoodGen_Project/finetuning_experiments/experiment_2_2decoder/captions_finetuned_final_checkpoint_food500_test.json"
checkpoint_csv = "/home/amal.saqib/Downloads/CV703_FoodGen_Project/finetuning_experiments/experiment_2_2decoder/captions_finetuned_final_checkpoint_food500_test.csv"
...
# === Load Model and Processor ===
processor = InstructBlipProcessor.from_pretrained(model_path, legacy=True, use_fast=False)
...
def generate_captions(images):
    ...
    # Use processor like you did in training
    inputs = processor(
        images=images,
    ...
    # Fix 2: Put the tensors on the correct device
    for k in inputs:
    ...
    return [final_caption_cleanup(cap) for cap in decoded]
...
for images, labels, filenames in dataloader:
    ...
    try:
        # Directly use original PIL images from dataset instead of ToPIL again
        pil_images = []
        for img in images:
            if isinstance(img, torch.Tensor):
                pil_images.append(transforms.ToPILImage()(img).convert("RGB"))
            else:
...
img = Image.open("/path/to/a/sample.jpg").convert("RGB")
inputs = processor(images=img, text="Describe this food.", return_tensors="pt").to(device)
out = model.generate(**inputs)
print(processor.batch_decode(out, skip_special_tokens=True))

# from PIL import Image
# import os, json, torch, time, csv, re
# from collections import Counter
# from torch.utils.data import Dataset, DataLoader
# from transformers import InstructBlipProcessor, InstructBlipForConditionalGeneration
# from datasets import load_dataset
...
#         if image.mode != "RGB":
#             image = image.convert("RGB")
#         return image, item["cat"], f"{idx}.jpg"
...
#     if not text.endswith("."):
#         text += "."
#     text = re.sub(r"\b(\w+)\s+(in|on|with)\s+(a\s+)?\1\b", r"\1", text, flags=re.IGNORECASE)
#     return text[0].upper() + text[1:]
...
#         for name, cap, label in zip(filenames, captions, labels):
#             full = f"{cap} (label: {label})"
#             captions_dict[name] = full
#             csv_rows.append([name, full])
#             print(f"[{len(captions_dict)}] {name}: {full}")
#     except Exception as e:
...
# # === Save ===
# with open(output_json, "w") as f_json:
#     json.dump(captions_dict, f_json, indent=2)
...
# print(f"✅ Done. Saved {len(captions_dict)} captions.")
